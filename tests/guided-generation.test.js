...
describe('guided generation flow wiring', () => {
  ...
  test('Upload starts guided generation and renders editable sample cards', () => {
    ...
    expect(upload).toContain('const handleAcceptSamples = useCallback')
    expect(upload).toContain('acceptStartedRef')
    ...
    expect(upload).toContain('Target cards')
    expect(upload).toContain('Limit')
  ...
  test('Projects opens rows and Review owns resumable generation actions', () => {
    ...
    expect(projects).toContain('Import JSON')
    expect(projects).toContain('accept=".json,.cardify.json"')
    expect(app).toContain("window.ipc.invoke('import-cardify-json'")
    expect(app).toContain("window.ipc.invoke('save-project'")
    expect(app).toContain('onImportProject={handleImportProject}')
    ...
    expect(app).toContain('sourceFilePath={uploadState.filePath}')
    expect(review).toContain('defaultAudioManifestCandidates')
    expect(review).toContain("window.ipc.invoke('load-audio-manifest', { path: manifestPath })")
    expect(read('electron/preload.js')).toContain("'read-audio-file'")
    expect(review).toContain('project-generation-grid')
    expect(review).toContain('Limit')
    expect(review).toContain('targetCardCount')
  ...
  test('iterative generation enforces target goal and repairs shortfalls once', () => {
    const main = read('electron/main.js')
    const app = read('src/App.jsx')
    const statusTile = read('src/components/StatusTile.jsx')

    expect(main).toContain('generationGoalStats(progress, currentCardCount)')
    expect(app).toContain('currentCardCount: mergeCardsUnique(cardsRef.current).length')
    expect(main).toContain("nextStatus = 'repairing_shortfall'")
    expect(main).toContain('shortfallRepairAttempted: true')
    expect(main).toContain("nextStatus = 'shortfall'")
    expect(main).toContain("['done', 'capped', 'shortfall'].includes(progress.status)")
    expect(main).toContain('function sanitizeCoverageText')
    expect(main).toContain('duplicate[-\\s]*safety')
    expect(app).toContain("'repairing_shortfall'")
    expect(app).toContain("result?.status === 'shortfall'")
    expect(statusTile).toContain('Repairing shortfall')
    expect(statusTile).toContain('Shortfall')
  })
  ...
  test('Electron exposes guided generation IPC channels', () => {
    ...
    expect(main).toContain('generationGoalQuestion()')
    expect(main).toContain("status: questions.length > originalQuestionCount ? 'questions' : result.status")
