...
function ensureGenerationGoal (goal, fallback = {}) {
  const normalized = normalizeGenerationGoal(goal)
  if (normalized.targetCardCount) return normalized
  return normalizeGenerationGoal(fallback)
}
...
function detectNumberedListCount (text) {
  ...
  const coverage = unique.length / last
  if (last >= 10 && coverage >= 0.8) {
    return normalizeGenerationGoal({
